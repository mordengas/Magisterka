...
def resolve_dirty_path(dataset_name, damage_level, damage_repeat):
    repeated_name = f"Data/{dataset_name}/{dataset_name}_10_50_prob_{damage_level}_r{damage_repeat}.csv"
...
def main():
    print("=== WALIDACJA 10-50 BEZ DATA LEAKAGE ===")
    print(f"Datasety: {[item['name'] for item in DATASETS_INFO]}")
    ...
    details_df.to_csv("wyniki_szczegolowe_10_50.csv", index=False)

    summary_df = summarize_results(details_df)
    summary_df.to_csv("wyniki_koncowe_10_50.csv", index=False)

    print("\nZapisano:")
    print("- wyniki_szczegolowe_10_50.csv")
    print("- wyniki_koncowe_10_50.csv")
