...
Rozdział~ten poświęcony jest elementom definiującym środowisko programistyczne, wykorzystane narzędzia oraz szczegółowej charakterystyce badanych zbiorów danych. W~pierwszej kolejności przedstawiono wykorzystane biblioteki języka Python, następnie omówiono parametry i~atrybuty każdego z~pięciu zbiorów, a~w dalszej części opisano metodologię symulacji kontrolowanych zanieczyszczeń oraz architekturę modularnego potoku przetwarzania danych.
...
\begin{itemize}
    \item \textbf{Pandas} \cite{PandasDocs} (wersja 2.x) – wykorzystywana do profilowania danych, identyfikacji brakujących pól, konwersji typów zmiennych oraz operacji tabelarycznych,
    \item \textbf{NumPy} \cite{NumpyDocs} – zapewniająca wydajne operacje macierzowe oraz funkcje losowania pseudolosowego z~użyciem generatora \texttt{default\_rng},
    \item \textbf{scikit-learn} \cite{SklearnDocs} – główny szkielet eksperymentalny; posłużyła do implementacji klasyfikatorów (\texttt{GaussianNB}, \texttt{MLPClassifier}, \texttt{RandomForestClassifier}), procedur podziału stratyfikowanego (\texttt{StratifiedKFold}), transformatorów czyszczących (\texttt{SimpleImputer}, \texttt{KNNImputer}, \texttt{StandardScaler}, \texttt{OneHotEncoder}) oraz obiektów potokowych (\texttt{Pipeline}, \texttt{ColumnTransformer}),
    \item \textbf{XGBoost} \cite{ChenGuestrin2016,XGBoostDocs} – wysoce zoptymalizowana implementacja algorytmu Extreme Gradient Boosting zintegrowana ze standardem interfejsu \texttt{scikit-learn},
    \item \textbf{Joblib} \cite{JoblibDocs} – biblioteka służąca do zrównoleglenia powtarzanych procedur walidacji krzyżowej na wielordzeniowych jednostkach obliczeniowych CPU,
    \item \textbf{Matplotlib} \cite{Hunter2007} oraz \textbf{Seaborn} \cite{Waskom2021} – narzędzia graficzne zastosowane do automatycznego generowania krzywych odporności modeli, macierzy korelacji oraz heatmap zysku $\Delta \mathrm{AUC}$.
\end{itemize}
...
Wszystkie operacje przetwarzania danych zorganizowano w~postaci modularnych obiektów \texttt{Pipeline} oraz \texttt{ColumnTransformer} z~biblioteki \texttt{scikit-learn}. Podejście to, rekomendowane przez Walkera \cite{Walker2021}, gwarantuje całkowitą hermetyzację transformacji, ponieważ parametry czyszczenia (progi IQR, mediany, średnie, parametry skalarów) są wyznaczane wyłącznie w~fazie \texttt{fit} na podzbiorze uczącym, a~następnie aplikowane do podzbioru testowego (\texttt{transform}), co eliminuje błąd wycieku danych (\textit{data leakage}).
...
\begin{enumerate}
    \item \textbf{$O_1$ (Eliminacja i~obsługa wartości odstających)}: identyfikacja anomalii regułą rozstępu międzykwartylowego ($3 \cdot \mathrm{IQR}$) i~ich konwersja do braków danych (\texttt{NaN}) przy użyciu własnej klasy transformatora \texttt{OutlierToNaNTransformer}.
...
\subsection{Implementacja własnych transformatorów w~standardzie scikit-learn}
Standardowa biblioteka \texttt{scikit-learn} nie zawiera wbudowanych modułów do selektywnego maskowania anomalii bez usuwania całych wierszy z~macierzy danych. Aby zintegrować regułę IQR oraz filtrację rzadkich kategorii wewnątrz modularnego potoku \texttt{Pipeline} bez ryzyka wycieku danych, zaimplementowano dwie dedykowane klasy transformatorów dziedziczące po \texttt{BaseEstimator} oraz \texttt{TransformerMixin}.

W~listingu~\ref{lst:outlier_transformer} przedstawiono strukturę algorytmiczną detektora \texttt{OutlierToNaNTransformer}. W~fazie \texttt{fit} klasa wyznacza progi odcięcia na danych treningowych, natomiast w~fazie \texttt{transform} zamienia wartości przekraczające wyznaczony zakres na \texttt{NaN}.

\begin{lstlisting}[caption={Pseudokod transformatora OutlierToNaNTransformer}, label={lst:outlier_transformer}]
Klasa OutlierToNaNTransformer(BaseEstimator, TransformerMixin):
    Inicjalizacja(numeric_columns, iqr_multiplier = 3.0, min_unique = 10):
        Zapisz parametry konfiguracyjne oraz zainicjalizuj bounds_ = {}

    Metoda fit(X_train):
        Dla kazdej kolumny numerycznej c w X_train:
            Jesli liczba unikalnych wartosci w c < min_unique:
                Pomin kolumne (traktuj jako ceche dyskretna)
            Q1 = kwartyl(X_train[c], 0.25)
            Q3 = kwartyl(X_train[c], 0.75)
            IQR = Q3 - Q1
            Jesli IQR > 0:
                lower = Q1 - iqr_multiplier * IQR
                upper = Q3 + iqr_multiplier * IQR
                bounds_[c] = (lower, upper)
        Zwroc self

    Metoda transform(X):
        Dla kazdej kolumny c w bounds_:
            lower, upper = bounds_[c]
            Dla kazdej wartosci x w kolumnie c:
                Jesli x < lower lub x > upper:
                    Ustaw wartosc x = NaN
        Zwroc zmodyfikowana macierz X
\end{lstlisting}

Z~kolei w~listingu~\ref{lst:rare_transformer} zaprezentowano pseudokod transformatora \texttt{RareCategoryToNaNTransformer}, którego zadaniem jest identyfikacja anomalnych lub rzadkich etykiet kategorycznych w~fazie uczenia i~ich neutralizacja w~fazie transformacji.

\begin{lstlisting}[caption={Pseudokod transformatora RareCategoryToNaNTransformer}, label={lst:rare_transformer}]
Klasa RareCategoryToNaNTransformer(BaseEstimator, TransformerMixin):
    Inicjalizacja(categorical_columns, min_frequency = 0.05):
        Zapisz parametry oraz zainicjalizuj valid_categories_ = {}

    Metoda fit(X_train):
        Dla kazdej kolumny kategorycznej c w X_train:
            Oblicz czestosc wystepowania kazdej kategorii w X_train[c]
            valid_categories_[c] = zbior kategorii o czestosci >= min_frequency
        Zwroc self

    Metoda transform(X):
        Dla kazdej kolumny c w valid_categories_:
            Dla kazdej wartosci x w kolumnie c:
                Jesli x nie jest puste i x nie nalezy do valid_categories_[c]:
                    Ustaw wartosc x = NaN
        Zwroc zmodyfikowana macierz X
\end{lstlisting}
...
\begin{itemize}
    ...
    \item \textbf{Perceptron Wielowarstwowy (MLP)}: sieć o~architekturze z~jedną warstwą ukrytą złożoną ze 100 neuronów (aktywacja ReLU, optymalizator Adam, wczesne zatrzymanie \texttt{early\_stopping=True}, \texttt{n\_iter\_no\_change=15}).
